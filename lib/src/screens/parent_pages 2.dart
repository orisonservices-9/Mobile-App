  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      Card(
        ...
        const ListTile(
            leading: Icon(Icons.schedule, color: orisonRed),
            title: Text('Next class'),
            subtitle: Text('Mathematics · 10:30 AM · Room 204')),
        const Divider(height: 1),
        ListTile(
            leading: const Icon(Icons.assignment_outlined, color: Colors.blue),
            title: Text(
                '${data.data.homework.where((e) => !e.completed).length} homework tasks due'),
            subtitle: const Text('Open Homework from Parent services')),
      ])),
      const SizedBox(height: 18),
      const _Heading('Latest notice'),
      const SizedBox(height: 8),
      Card(
          child: ListTile(
              title: Text(data.data.notices.first.title,
                  style: const TextStyle(fontWeight: FontWeight.w700)),
              subtitle: Text(data.data.notices.first.body),
...
class _HelpDeskPageState extends State<HelpDeskPage> {
  ...
  void _submit() {
    ...
    context.read<ParentBloc>().add(HelpRequestSubmitted(
          kind: callback ? 'Academic callback' : 'App support',
          category: callback ? _academicCategory : _appCategory,
          description: details,
          priority: callback ? 'Normal' : _priority,
          preferredTime: callback ? _callbackTime : null,
              ...
              Row(children: [
                Expanded(
                  child: _HelpServiceCard(
                    ...
                    title: 'Academic callback',
                    subtitle: 'Talk with the school academic team',
                    color: const Color(0xFF3157C8),
...
class _AcademicCallbackForm extends StatelessWidget {
  ...
  Widget build(BuildContext context) => _HelpFormShell(
        color: const Color(0xFF3157C8),
        icon: Icons.phone_callback_rounded,
        title: 'Request an academic callback',
        subtitle:
            'A coordinator or teacher will call the registered parent number.',
        children: [
          DropdownButtonFormField<String>(
            initialValue: category,
            decoration: const InputDecoration(
              labelText: 'Academic concern',
...
class _HelpRequestCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final callback = request.kind == 'Academic callback';
